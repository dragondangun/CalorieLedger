...
public sealed class AdaptivePlanDataQualityEvaluatorTests {
    ...
    public void Evaluate_WithSufficientData_ReturnsAvailableResult() {
        ...
        var intakeEntries = Enumerable.Range(0, 10)
            .Select(
                day => new DailyEnergyIntakeEntry(
                    Date: startDate.AddDays(day),
                    CaloriesKcal: 2000m + day * 10m,
                    IsComplete: true)
            ).ToArray();
    ...
    public void Evaluate_WithTooFewCompleteIntakeDays_ReturnsIssue() {
        ...
        var bodyMeasurements = Enumerable.Range(0, 8)
            .Select(day => CreateBodyMeasurement(startDate.AddDays(day * 13 / 7), 80m - day * 0.1m))
            .ToArray();

        var intakeEntries = Enumerable.Range(0, 9)
            .Select(
                day => new DailyEnergyIntakeEntry(
                    Date: startDate.AddDays(day),
                    CaloriesKcal: 2000m,
        ...
        Assert.True(result.HasIssue(AdaptivePlanDataIssue.InsufficientCompleteIntakeDays));
    ...
    public void Evaluate_IgnoresIncompleteIntakeDays() {
        ...
        var bodyMeasurements = Enumerable .Range(0, 8)
            .Select(
                day => CreateBodyMeasurement(
                    startDate.AddDays(
                        day * 13 / 7),
                    80m - day * 0.1m))
            .ToArray();

        var completeEntries = Enumerable.Range(0, 8)
            .Select(
                day => new DailyEnergyIntakeEntry(
                    Date: startDate.AddDays(day),
                    CaloriesKcal: 2000m,
        ...
        var incompleteEntries = Enumerable.Range(8, 4)
            .Select(
                day => new DailyEnergyIntakeEntry(
                    Date: startDate.AddDays(day),
                    CaloriesKcal: 1200m,
        ...
        var result = AdaptivePlanDataQualityEvaluator.Evaluate(
            bodyMeasurements,
            completeEntries.Concat(incompleteEntries),
            asOfDate
        );
    ...
    public void Evaluate_WithShortObservationPeriod_ReturnsIssue() {
        ...
        Assert.True(result.HasIssue(AdaptivePlanDataIssue.ObservationPeriodTooShort));
    }
    ...
    public void Evaluate_WithDuplicateIntakeDates_ThrowsArgumentException() {
        ...
        Assert.Throws<ArgumentException>(
            () => AdaptivePlanDataQualityEvaluator.Evaluate(
                bodyMeasurements: Array.Empty<BodyMeasurementEntry>(),
                intakeEntries: intakeEntries,
                asOfDate: date.AddDays(13)
            )
        );
    }
    [Fact]
    public void Evaluate_FutureBodyMeasurements_DoNotAffectResult() {
        var startDate = new DateOnly(2026, 7, 1);
        var asOfDate = startDate.AddDays(13);

        var bodyMeasurements = Enumerable.Range(0, 14)
            .Select(
                day => CreateBodyMeasurement(
                    startDate.AddDays(day),
                    80m - day * 0.1m
        ...
        Assert.True(result.WeightTrend.EstimatedWeeklyChangeKg < 0m);
    }

    [Fact]
    public void Evaluate_FutureIntakeEntries_DoNotAffectResult() {
        var startDate = new DateOnly(2026, 7, 1);
        var asOfDate = startDate.AddDays(13);
